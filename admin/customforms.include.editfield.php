...
function admin_customformsBuild($data,$db) {
	...
	$statement = $db->prepare('getFieldById', 'customform');
	$statement->execute(array(':id' => $data->action[3]));
	$data->output['customfield'] = $field = $statement->fetch();
	if($field === false){
		$data->output['abort'] = true;
