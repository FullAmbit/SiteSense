...
function theme_displayComments($Comments){
	...
	foreach($Comments as $Comment){
		$Comment['content'] = '<p>' . str_replace(array("\r\n", "\n", "\r"), '</p><p>', htmlspecialchars($Comment['content'])) . '<p>';
		while(strpos($Comment['content'], '<p></p>') !== false){
			$Comment['content'] = str_replace('<p></p>', '', $Comment['content']);
		}
		echo '
			<li>
				<blockquote>
					',$Comment['content'],'
					<cite>
						Posted by ',$Comment['commenter'],' on ', date('F j, Y \a\t g:i a', strtotime($Comment['time'])),'
					</cite>
				</blockquote>
			</li>
		';
