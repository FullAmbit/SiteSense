...
function modules_admin_common_runUpgrader($data,$db,$to,$from,$shortName,$file){
	common_include('modules/'.$shortName.'/updaters/'.$file);
	$functionName='admin_'.$shortName.'_updater_';
	$functionName.=preg_replace('/\D/','',$from).'_';
	$functionName.=preg_replace('/\D/','',$to);
	if (!function_exists($functionName)){
		return FALSE;
	}else{
		return call_user_func_array($functionName,array($data,$db));
	}
}
